...
const DesignCard = ({ data }) => {
    return (
        <div className="relative w-[315px] h-[236px] bg-black rounded-lg group cursor-pointer">
            ...
            <div className="absolute inset-0 transition-shadow duration-300 shadow-lg opacity-0 group-hover:opacity-100 group-hover:shadow-2xl"></div>
            <div className='absolute left-0 w-full h-4 -bottom-[28px] z-10 flex gap-2 justify-between'>
                <div className='flex items-center gap-2'>
                    <img src={data?.designer?.image} className='w-6 h-6 rounded-full' />
                    <p className='overflow-hidden text-sm font-medium text-ellipsis whitespace-nowrap'>{data?.designer?.name}</p>
                    {data?.designer?.isCompany ? <div className='text-white bg-gray-400 h-[16px] font-bold text-[10px] p-[3px] rounded-[3px] leading-[1]'>TEAM</div> : <div className='text-white bg-gray-400 h-[16px] font-bold text-[10px] p-[3px] rounded-[3px] leading-[1]'>PRO</div>}
                </div>
